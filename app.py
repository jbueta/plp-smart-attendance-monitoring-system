...
from db_connect import Database, EmployeeModel, StudentModel
...
# [ANNOUNCEMENT FEATURE] - Blueprint Registration
...
def get_kiosk_data_with_live_feed(limit=6, target_event=None):
    ...
    # [ANNOUNCEMENT FEATURE] - Fetch real bulletin
    try:
        from announcement_models import AnnouncementModel
        model = AnnouncementModel()
        # Use date.today().isoformat() for consistency
        ...
        app.logger.error(f"Error fetching bulletin for kiosk (event={target_event}): {e}")

    # Ensure defaults exist to prevent Template errors
...
def kiosk_entrance():
    ...
    return render_template(
        ...
        kiosk_data=get_kiosk_data_with_live_feed(target_event="General Attendance"),
    )
...
def kiosk_exit():
    session.clear()
    return render_template("kiosk_exit.html", kiosk_data=get_kiosk_data_with_live_feed(target_event="General Attendance"))
...
def kiosk_employee():
    ...
    return render_template(
        ...
        kiosk_data=get_kiosk_data_with_live_feed(target_event=event_name),
    )
...
def visitor_checkin():
    ...
    if not name or not purpose:
        if is_ajax:
            return jsonify({"success": False, "message": "Name and purpose are required."}), 400
    ...
    if not conn:
        if is_ajax:
            return jsonify({"success": False, "message": "Visitor database is unavailable."}), 500
        ...
        if result.get("success"):
            return jsonify({
                ...
                "visitor": {
                    "name": name,
                    "purpose": purpose,
                    "visitor_id": result.get("visitor_id", "VT-00000"),
                },
            })
...
def manage_events():
    ...
    for e in events:
        try:
            dt = datetime.strptime(str(e.get("date", "")), "%Y-%m-%d")
            e["formatted_date"] = dt.strftime("%B %d, %Y").replace(" 0", " ")
        except ValueError:
            e["formatted_date"] = e.get("date", "")
...
@app.route("/api/backend/<path:subpath>", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
def backend_proxy(subpath):
    """Transparent proxy: forwards kiosk/frontend JS calls to app_extension (port 5001)."""
    try:
        ...
        app.logger.error("Backend proxy error for /%s: %s", subpath, err)
        return jsonify({"success": False, "message": "Backend service unavailable."}), 503
...
def upload_employees():
    ...
    try:
        ...
        df = df.map(lambda value: clean(value) if isinstance(value, str) else value)
...
def download_student_upload_template():
    ...
    writer.writerow(["23-00001", "JUAN DELA CRUZ", "BSIT", "Outside"])
...
def upload_students_preview():
    ...
    file_ext = os.path.splitext(file.filename)[1].lower()
    if file_ext not in {".xls", ".xlsx", ".csv"}:
        return jsonify({"success": False, "error": "Invalid file type. Please upload Excel or CSV."}), 400

    try:
        import pandas as pd
        reader = pd.read_csv if file_ext == ".csv" else pd.read_excel
        df = reader(file, dtype=str)
        ...
        for idx, row in df.iterrows():
            sid = str(row.get("STUDENT ID") or "").strip()
            ...
            preview_rows.append({
                ...
                "student_name": name,
                "course_name": course,
                "status": str(row.get("STATUS") or "Outside").strip(),
        ...
        return jsonify({
            ...
            "optional_columns": ["COURSE", "STATUS"]
        })
    except Exception as err:
        return jsonify({"success": False, "error": str(err)}), 500
...
@login_required
def upload_students_commit():
    data = request.get_json(silent=True) or {}
    rows = data.get("rows", [])
    if not rows:
        return jsonify({"success": False, "error": "No rows to commit"}), 400
    ...
    try:
        for row in rows:
            ...
            if result.get("success"):
                ...
                errors.append(f"ID {row.get('student_id')}: {result.get('error')}")
...
def update_student():
    ...
    try:
        ...
        if not cursor.fetchone():
            return jsonify({"success": False, "error": "Course not found."}), 400
        ...
        cursor.execute(
            ...
            WHERE student_id = %s
            """,
            (student_name, course_id, student_id),
        )
        conn.commit()
...
def delete_student():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT user_id FROM students WHERE student_id = %s LIMIT 1",
            (student_id,),
        )
